...
func TestAVLHashTree_DefaultInsertHashesKey(t *testing.T) {
	...
	got, err := tree.SearchByKey(key)
	if err != nil {
		t.Fatalf("SearchByKey returned error: %v", err)
	}
	...
	proof, err := tree.GenerateInclusionExclusionProofByKey(key)
	if err != nil {
		t.Fatalf("GenerateInclusionExclusionProofByKey returned error: %v", err)
	}
	if !proof.Found {
	...
	if err := tree.DeleteByKey(key); err != nil {
		t.Fatalf("DeleteByKey returned error: %v", err)
	}
	if tree.Root != nil {
